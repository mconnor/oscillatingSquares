...
define(["jquery", "jquery.purls", "TweenMax", "jqueryui"],
	 function( $ , $purl, TweenMax) {
		...
		onResize = function ( _$container ){
			...
			if (tweenAlpha) {
				TweenMax.set( _$container.children('div.bip') , { x:origX, y:origY, opacity:0} );
			} else {
				TweenMax.set(  _$container.children('div.bip')  , { x:origX, y:origY} );
			}
		...
		publicInit = function (elem) {
			...
			$container.resizable({
 				 start: function( event, ui ) {
 				 	...
 				 	TweenMax.to( $bips , 0.5,  { opacity:0 });
